...
export const createDailyEntry = asyncHandler(async (req, res) => {
  const entry = await dailyHifzService.createEntry(req.tenantId, req.body, req.branchScope);
...
export const getDailyEntries = asyncHandler(async (req, res) => {
  const entries = await dailyHifzService.getEntries(req.tenantId, req.query, req.branchScope);
...
export const getDailyEntryById = asyncHandler(async (req, res) => {
  const entry = await dailyHifzService.getEntryById(req.tenantId, Number(req.params.id), req.branchScope);
...
export const updateDailyEntry = asyncHandler(async (req, res) => {
  const entry = await dailyHifzService.updateEntry(req.tenantId, Number(req.params.id), req.body, req.branchScope);
...
export const deactivateDailyEntry = asyncHandler(async (req, res) => {
  const entry = await dailyHifzService.deactivateEntry(req.tenantId, Number(req.params.id), req.branchScope);
