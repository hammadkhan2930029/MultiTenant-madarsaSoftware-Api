...
export const createSiparaEntry = asyncHandler(async (req, res) => {
  const entry = await siparaHifzService.createEntry(req.tenantId, req.body);
...
export const getSiparaEntries = asyncHandler(async (req, res) => {
  const entries = await siparaHifzService.getEntries(req.tenantId, req.query);
...
export const getSiparaEntryById = asyncHandler(async (req, res) => {
  const entry = await siparaHifzService.getEntryById(req.tenantId, Number(req.params.id));
...
export const updateSiparaEntry = asyncHandler(async (req, res) => {
  const entry = await siparaHifzService.updateEntry(req.tenantId, Number(req.params.id), req.body);
...
export const deactivateSiparaEntry = asyncHandler(async (req, res) => {
  const entry = await siparaHifzService.deactivateEntry(req.tenantId, Number(req.params.id));
