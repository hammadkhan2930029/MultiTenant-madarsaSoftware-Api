...
export const createWeeklyEntry = asyncHandler(async (req, res) => {
  const entry = await weeklyHifzService.createEntry(req.tenantId, req.body);
...
export const getWeeklyEntries = asyncHandler(async (req, res) => {
  const entries = await weeklyHifzService.getEntries(req.tenantId, req.query);
...
export const getWeeklyEntryById = asyncHandler(async (req, res) => {
  const entry = await weeklyHifzService.getEntryById(req.tenantId, Number(req.params.id));
...
export const updateWeeklyEntry = asyncHandler(async (req, res) => {
  const entry = await weeklyHifzService.updateEntry(req.tenantId, Number(req.params.id), req.body);
...
export const deactivateWeeklyEntry = asyncHandler(async (req, res) => {
  const entry = await weeklyHifzService.deactivateEntry(req.tenantId, Number(req.params.id));
