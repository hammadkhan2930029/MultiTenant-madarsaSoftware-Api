...
export const createMonthlyEntry = asyncHandler(async (req, res) => {
  const entry = await monthlyHifzService.createEntry(req.body);
...
export const getMonthlyEntries = asyncHandler(async (req, res) => {
  const entries = await monthlyHifzService.getEntries(req.query);
...
export const getMonthlyEntryById = asyncHandler(async (req, res) => {
  const entry = await monthlyHifzService.getEntryById(Number(req.params.id));
...
export const updateMonthlyEntry = asyncHandler(async (req, res) => {
  const entry = await monthlyHifzService.updateEntry(Number(req.params.id), req.body);
...
export const deactivateMonthlyEntry = asyncHandler(async (req, res) => {
  const entry = await monthlyHifzService.deactivateEntry(Number(req.params.id));
