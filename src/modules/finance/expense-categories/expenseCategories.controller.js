...
export const getExpenseCategories = asyncHandler(async (req, res) => {
  const categories = await expenseCategoriesService.getCategories(req.tenantId, req.query, req.branchScope);
...
export const getExpenseCategoryById = asyncHandler(async (req, res) => {
  const category = await expenseCategoriesService.getCategoryById(req.tenantId, Number(req.params.id), req.branchScope);
