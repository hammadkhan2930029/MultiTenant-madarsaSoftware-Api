...
export const getHeads = asyncHandler(async (req, res) => {
  const heads = await headsService.getHeads(req.tenantId, req.query, req.branchScope);
...
export const getHeadById = asyncHandler(async (req, res) => {
  const head = await headsService.getHeadById(req.tenantId, Number(req.params.id), req.branchScope);
