...
export const createSalaryEntry = asyncHandler(async (req, res) => {
  const entry = await salariesService.createEntry(req.body);
...
export const getSalaryEntries = asyncHandler(async (req, res) => {
  const entries = await salariesService.getEntries(req.query);
...
export const getSalaryEntryById = asyncHandler(async (req, res) => {
  const entry = await salariesService.getEntryById(Number(req.params.id));
...
export const updateSalaryEntry = asyncHandler(async (req, res) => {
  const entry = await salariesService.updateEntry(Number(req.params.id), req.body);
...
export const deactivateSalaryEntry = asyncHandler(async (req, res) => {
  const entry = await salariesService.deactivateEntry(Number(req.params.id));
